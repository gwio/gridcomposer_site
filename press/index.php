...
foreach( $xml->children() as $child )
{
	switch( $child->getName() )
	{
		...
		case("platform"):
			...
			foreach( $child->children() as $subchild )
			{
				$features[$i][$subchild->getName()] = $subchild;
				$i++;
			}
			break;
			case("features"):
	...
	function parseLink($uri)
	{
      ...
      include_once('../analyticstracking.php');
			...
			<li><a href="#history">'. tl('Features') .'</a></li>
			...
			<a href="http://' . parseLink(DEVELOPER_WEB) . '">'. parseLink(DEVELOPER_TITLE) .'</a><br/>
			...
			<p>
			<strong>'. tl('Platform:') .'</strong><br/>
			' . PLATFORM . '
			</p>
			<p>
			<strong>'. tl('Category:') .'</strong><br/>
			' . CATEGORY . '
			</p>
			<p>
			<strong>'. tl('Price:') .'</strong><br/>
			' . PRICE . '
			</p>
			<strong>'. tl('StoreLink:') .'</strong><br/>
			<a href="https://itunes.apple.com/us/app/gridcomposer/id993489006?ls=1&mt=8">iTunes Appstore</a>
			</p>
			<strong>'. tl('Release:') .'</strong><br/>
			' . RELEASE_DATE . '
			</p>
			...
			<h2 id="history">'. tl('Features') .'</h2>';

echo '<ul>';
			for( $i = 0; $i < count($features); $i++ )
			{
				$header = $text ="";
				foreach( $features[$i]['feature']->children() as $child ){
